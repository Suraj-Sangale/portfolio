...
// Sandboxed upload target directory for local fallback
...
/**
 * Check whether Supabase is configured in environment
 */
...
/**
 * Ensure sandboxed upload directory exists (used in local storage fallback)
 */
export function ensureUploadDir() {
  try {
    ...
    // In serverless read-only environments (like Vercel), this may fail silently
    console.warn("Local upload directory cannot be created:", err.message);
  }
}

/**
 * Sanitize a string to be a safe base name (without extension)
 */
...
/**
 * Verify whether path resolves safely strictly within the upload sandbox (local fallback)
 */
...
/**
 * Resolve duplicate filename against a set/list of existing file names
 */
export function resolveDuplicateNameFromList(existingNames, baseName, ext, strategy = "auto-rename") {
  ...
  if (strategy === "overwrite" || !nameSet.has(initialName)) {
    return initialName;
  }
...
/**
 * Generate a unique filename if collision occurs on local disk
 */
