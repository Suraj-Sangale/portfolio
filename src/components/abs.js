      ...
      <div className='about__container '>
        ...
        <div className='about__data '>
          <div className='about__info'>
            ...
            <div className="grid grid-cols-4  m-5 justify-center	 md:grid-cols-4 gap">
              {images.map((item, index) => (
                <div key={index} className="flex justify-center items-center h-22 md:h-48 ">
                  <img src={item.imgUrl} alt={`Image ${index.imgUrl + 1}`} className="max-h-20 max-w-24" />
                </div>
              ))}
            </div>
